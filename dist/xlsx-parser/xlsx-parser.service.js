...
let XlsxParserService = class XlsxParserService {
            ...
            sheet_name_list.map((sheetName) => {
                ...
                if (sheetName !== 'Status Overview' && sheetName !== 'Overview') {
                    ...
                    for (let x = 0; x < xlsxFileAsJsonObject.length; x++) {
                        if (xlsxFileAsJsonObject[x]['__EMPTY'] ===
                    ...
                    const newMeasure = {
                        ...
                        id,
                        title: sheetName,
                        name: xlsxFileAsJsonObject[2][sheetName],
                        description,
                        time: xlsxFileAsJsonObject[2]['__EMPTY_11'],
                        lastUpdate: xlsxFileAsJsonObject[2]['__EMPTY_15'],
                        focusArea: focusAreaNames[xlsxFileAsJsonObject[2]['__EMPTY_2']],
                        focusAreaFull: xlsxFileAsJsonObject[2]['__EMPTY_2'],
                        measureLead,
                        measureSponsor,
                        lineOrgSponsor,
                        solutionManager,
                    ...
                    measure
                        ...
                        .then(async (savedMeasure) => {
                        const artefacts = this.getArtefactsFromLinesArray(xlsxFileAsJsonObject, sheetName);
                        const savedArtefact_IDs = [];
                        artefacts.map((art) => {
                            const toSave = {
                                id: art[sheetName],
                                description: art['__EMPTY'],
                                progress: art['__EMPTY_3'],
                                budget: art['__EMPTY_5'] ? art['__EMPTY_5'] : '',
                                achievement: art['__EMPTY_6'],
                                work: art['__EMPTY_13'],
                            };
    ...
    getArtefactsFromLinesArray(sheet, sheetName) {
        return sheet.filter((line) => {
            const firstKey = Object.keys(line)[0];
            if (firstKey === sheetName) {
    ...
    async parseKPI() {
        const workbook = XLSX.readFile(path_1.resolve(globalVars_1.fileNames.xlsx_file_dir, globalVars_1.fileNames.kpi_file_2));
        const overview_object = workbook.Sheets['Plan view'];
    ...
    async parseBudgetMonths() {
        ...
        const totalApprovedBudget = overview_object['M12']['v'];
        const month_columns = [];
