...
import { fileNames, FOCUS_AREA_NAMES, rootPath } from '../globalVars';
import '../types';
import 'src/types';
import {
  Risk,
...
export class XlsxParserService {
  ...
  async triggerParsing(): Promise<string> {
    ...
    try {
      if (fs.existsSync(rootPath + "/data/" + "status_report.xlsx")) {
        ...
        await fs.copyFile(rootPath + "/src/realData/" + "status_report.xlsx", rootPath + "/data/" + "status_report.xlsx", (err) => {
          if (err) { throw err }
          else {
      ...
      if (fs.existsSync(rootPath + "/data/" + "budget_report.xlsx")) {
        ...
        await fs.copyFile(rootPath + "/src/realData/" + "budget_report.xlsx", rootPath + "/data/" + "budget_report.xlsx", (err) => {
          if (err) { throw err }
          else {
            console.log('source.txt was copied to destination.txt');
            budgetReportExists = true
      ...
      if (fs.existsSync(rootPath + "/data/" + "KPI-report_1.xlsx")) {
        ...
        await fs.copyFile(rootPath + "/src/realData/" + "KPI-report_1.xlsx", rootPath + "/data/" + "KPI-report_1.xlsx", (err) => {
          if (err) { throw err }
          else {
            console.log('source.txt was copied to destination.txt');
            kpiReportExists = true
      ...
      if (fs.existsSync(rootPath + "/data/" + "test_data.xlsx")) {
        ...
        await fs.copyFile(rootPath + "/src/realData/" + "test_data.xlsx", rootPath + "/data/" + "test_data.xlsx", (err) => {
          if (err) { throw err }
          else {
            console.log('source.txt was copied to destination.txt');
            datatExists = true
      ...
      if (fs.existsSync(rootPath + "/data/" + "budget_past.xlsx")) {
        ...
        await fs.copyFile(rootPath + "/src/realData/" + "budget_past.xlsx", rootPath + "/data/" + "budget_past.xlsx", (err) => {
          if (err) { throw err }
          else {
            console.log('source.txt was copied to destination.txt');
            pastBudgetsExists = true
    ...
    if (parsedNewData) {
      ...
      if (measuresOLD.length > 0) { // old data exists in DB
  ...
  async deleteData(temp) {
    return new Promise((resolve, reject) => {
      setTimeout(async () => {
        if (false) reject(new Error('no first name passed in!'))
        if (temp) {
  ...
  async moveFilesAfterParsing() {
    fs.readdir(rootPath + '/data/', (err, files) => {
      files.forEach(file => {
        fs.rename(rootPath + '/data/' + file, rootPath + '/src/realData/' + file, function (err) {
          if (err) {
            throw err
          } else {
  ...
  async createOverview(temp): Promise<InitialOverview> {
    const main_file = temp ? rootPath + "/data/" + "test_data.xlsx" : resolve(fileNames.xlsx_file_dir, fileNames.main_file)
    const status_report = temp ? rootPath + "/data/" + "status_report.xlsx" : resolve(fileNames.xlsx_file_dir, fileNames.status_report)
    const budget_file = temp ? rootPath + "/data/" + "budget_report.xlsx" : resolve(fileNames.xlsx_file_dir, fileNames.budget_file)
    const kpi_file_1 = temp ? rootPath + "/data/" + "KPI-report_1.xlsx" : resolve(fileNames.xlsx_file_dir, fileNames.kpi_file_1)
    ...
    if (excelSheet) {
      ...
      const KPIprogressOfAllMeasures =
        this.getKPIProgressData(kpi_file_1)
      let sum = 0;
  ...
  async parse(temp: boolean): Promise<string> {
              ...
              if (typeof (xlsxFileAsJsonObject[x + 3]['__EMPTY_8']) !== 'undefined') {
                ...
                if (typeof (xlsxFileAsJsonObject[x + 6]['__EMPTY_8']) !== 'undefined') {
                  ...
                  if (xlsxFileAsJsonObject[x + 7] && typeof (xlsxFileAsJsonObject[x + 7]) !== 'undefined') {
                    if (!("" + xlsxFileAsJsonObject[x + 7]['__EMPTY_8']).includes("Medium")) {  // risk 4 or text under risks area?
  ...
  async parseKPI(temp): Promise<string> {
    const kpi_file_1 = temp ? rootPath + "/data/" + "KPI-report_1.xlsx" : resolve(fileNames.xlsx_file_dir, fileNames.kpi_file_1)
  ...
  async parseBudgetMonths(temp): Promise<string> {
  ...
  getRisk(line: number, xlsxFileAsJsonObject) {
    let risk: Risk = {
      risk: '',
      description: '',
      criticality: '',
